...
class FluidMechanicsTimePressureVariationsInAnIdealGasViewController: UIViewController {
    ...
    @IBOutlet weak var myTextField1: UITextField! // Initial Pressure (Pa)
    @IBOutlet weak var myTextField2: UITextField! // Final Pressure (Pa)
    @IBOutlet weak var myTextField3: UITextField! // Volume of Tank (m^3)
    @IBOutlet weak var myTextField4: UITextField! // Volumetric Flow Rate (m^3 / s)
    ...
    override func viewDidLoad() {
        ...
        label.text = "Time Taken to Evacuate \n a Tank with an Ideal Gas (s)"
    ...
    @IBAction func myButton2(_ sender: Any) {
        ...
        let firstValue = Double(myTextField1.text!) // Initial Pressure (Pa)
        let secondValue = Double(myTextField2.text!) // Final Pressure (Pa)
        let thirdValue = Double(myTextField3.text!) // Volume of Tank (m^3)
        let fourthValue = Double(myTextField4.text!) // Volumetric Flow Rate (m^3 / s)
        
        
        if firstValue != nil && secondValue != nil && thirdValue != nil && fourthValue != nil {
            
            
            let TimeTakenToEvacuateTankConstant1 = Double(log(secondValue! / firstValue!))
            let TimeTakenToEvacuateTank = Double(-thirdValue! * TimeTakenToEvacuateTankConstant1 / fourthValue!)
            
            
            myLabel1.text = "Time required to evacuate tank from its initial pressure = \(TimeTakenToEvacuateTank) s"
